...
export function initializeLobby() {
  ...
  const updateUI = () => {
    ...
    // Mostra a seção de online se houver 2 jogadores
    if (myRoom.players.length > 1) showOnlineView();
    ...
    // Redireciona para multiplayer apenas se ambos estiverem prontos
    if (myRoom.players.length === 2 && myRoom.playersReady.every(p => p.ready)) {
  ...
  socket.on('online_users', (users) => {
    ui.playersList.innerHTML = users.length === 0
      ? '<li class="no-players">Nenhum jogador online.</li>'
      : users.filter(u => u.id !== myData._id)
             .map(player => `
              <li>
                <span class="status online"></span>
                <span class="emoji">👤</span>
                <span class="name">${player.username}</span>
                <button class="btn-status invite-btn" data-player-id="${player.id}">Convidar</button>
              </li>
             `).join('');
  });
  ...
  ui.bell?.addEventListener('click', () => {
    ui.notificationPanel.style.display =
      ui.notificationPanel.style.display === 'block' ? 'none' : 'block';
  ...
  ui.notificationList?.addEventListener('click', async (e) => {
    ...
    if (target.classList.contains('accept')) {
      const res = await fetch(`/rooms/join/${roomId}`, { method: 'POST', credentials: 'include' });
      if (res.ok) {
        myRoom = await res.json();
        updateUI();
      }
    } else {
